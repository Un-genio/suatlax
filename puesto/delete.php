...
$Id = $_POST['Id'];
if($Id){ 
 $sql = "DELETE FROM tbls_puesto WHERE id_puesto='$Id'";
 if($connect->query($sql) === TRUE){
    $result=$connect->query("SHOW TABLE STATUS FROM $name LIKE 'tbls_puesto' "); 
    $fila=$result->fetch_array();
    $ai=(int)$fila['Auto_increment'];
    $datos=$connect->query("SELECT MAX(id_puesto) FROM tbls_puesto");
    ...
    if($comparar==1){
        ...
        $valid['success'] = false;
        $sql =$connect->query("ALTER TABLE tbls_puesto AUTO_INCREMENT=".$comparar."");
        if($sql === TRUE){
            $valid['success'] = true;
